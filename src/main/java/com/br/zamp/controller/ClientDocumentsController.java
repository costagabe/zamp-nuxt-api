...
@CrudPermission({
  ...
  @CrudPermissionType(
      permission = Permission.READ_ALL_CLIENT_DOCUMENTS,
      method = CrudMethod.READ_ALL),
...
public class ClientDocumentsController {
  ...
  @PostMapping
  public ResponseEntity<ReadAndUpdateClientDocumentDTO> create(
      @PathVariable("clientId") UUID clientId, @Valid @RequestBody CreateClientDocumentDTO dto) {
  ...
  @GetMapping
  public ResponseEntity<CustomPage<ReadAndUpdateClientDocumentDTO>> findAll(
      @PathVariable("clientId") UUID clientId, Pageable pageable) {
    var entities = clientDocumentService.findAll(clientId, pageable);
    Page<ReadAndUpdateClientDocumentDTO> pageDto =
        entities.map(clientDocumentMapper::toReadAndUpdateDTO);
  ...
  public ResponseEntity<?> update(
      @Valid @RequestBody ReadAndUpdateClientDocumentDTO dto, @PathVariable UUID id) {
