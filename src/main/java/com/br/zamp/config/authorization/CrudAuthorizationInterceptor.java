...
public class CrudAuthorizationInterceptor implements HandlerInterceptor {
      ...
      if (crudAnnotation.isPresent() && !shouldSkipAnnotationCheck(handler, crudAnnotation.get())) {
        ...
        Set<String> permissionSet =
            populatePermissionSet(
                ((HandlerMethod) handler).getMethod().getName(), crudAnnotation.get(), level);
        ...
        if (canContinue(permissionSet)) {
          ...
          throw new AuthorizationServiceException(
              String.format("Usuário sem autorização (%s)", p.getDescription()));
        }
      }
      return true;
  ...
  private boolean isMethodListed(String method, CrudPermission crudAnnotation) {
    ...
    return permissionTypes.stream().anyMatch(v -> v.method().getDescription().equals(method));
  }
  ...
  private Set<String> populatePermissionSet(
      String method, CrudPermission crudAnnotation, Integer userLevel) {
    List<CrudPermissionType> permissionTypes = Arrays.stream(crudAnnotation.value()).toList();
  ...
  private <A extends Annotation> A findAnnotation(
      Class<?> targetClass, @NonNull Class<A> annotationClass) {
